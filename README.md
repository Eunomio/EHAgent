...
- VLM首次发现中高等级通道风险时，生成可追溯的主动关怀事件
- 睡眠助手连续两晚偏离个人基线后，主动询问本人感受和可能原因
- 跌倒、近跌倒、跌倒担忧和活动回避只从日常对话中提取为候选事实
- 候选事实必须由本人点击确认后才进入画像；本人可以拒绝或删除
- 首页显示当前主动关怀及触发原因，对话页支持稍后再说和暂停主动关怀
- 固定自助资源包括放松练习、睡前白噪音、睡前准备和担忧整理，并记录开始情况
- 默认静默时段为21:30—08:00，普通主动关怀每天最多2次

Android 当前仅保留 actual 版本，使用 `testActualDebugUnitTest assembleActualDebug` 测试和构建。demo 版本、内置走廊视频、自动导入演示睡眠及客户端演示操作入口已移除。共用后端的开发数据接口仍保留，生产环境继续禁止演示数据操作。
