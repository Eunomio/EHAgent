...
当前 Android 版本为 `0.10.2`，支持 Android 8.0 及以上系统。
...
- 视觉模型分别判断剩余通行空间和绊倒风险，通道边缘且不影响行走的日常物品保持安全状态
...
- LLM 生成通道整改建议、睡眠小结和反馈摘要；服务未配置或调用失败时使用固定模板
- 支持联系家人、家属协助请求、设备状态和隐私设置

页面只展示真实设备数据。设备尚未连接或数据尚未同步时，会显示“等待连接”或“暂未同步”。
...
- [模型、数据采集与上传方法](docs/MODEL_AND_DATA_GUIDE.md)
- [后端接口说明](docs/API.md)
