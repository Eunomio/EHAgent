...
data class SleepCard(val headline: String = "睡眠数据暂未同步", val duration: Int? = null, val respiratoryRate: Double? = null, val heartRate: Double? = null, val bedExitCount: Int? = null)
...
class ProductApi(private val baseUrl: String) {
    ...
    suspend fun dashboard(): Dashboard {
        ...
        return Dashboard(
            ...
            sleep = SleepCard(sleep.optString("headline"), summary?.optInt("duration_minutes"), summary?.optionalDouble("respiratory_rate"), summary?.optionalDouble("heart_rate"), summary?.takeIf { it.has("bed_exit_count") && !it.isNull("bed_exit_count") }?.optInt("bed_exit_count")),
