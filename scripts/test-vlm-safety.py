...
def prompt_text(label: dict[str, Any], walkway: dict[str, Any]) -> str:
    ...
    return (
        ...
        "采用‘物理致害潜势—情境可规避性’原则：先判断物品是否进入实际行走轨迹、落脚区、"
        "转弯区、门口或门槛，再判断从可能接近方向能否提前察觉，以及发现后是否仍有足够反应"
        "距离和安全避让空间。情境可规避性同时包含可察觉性和可避让性。"
        "杂物数量、物品尺寸、画面面积或视觉杂乱程度都不能单独决定风险。大量醒目物品即使"
        "容易察觉，只要压缩通道或迫使绕行仍属于风险；单个低矮小物品若位于拐角后、遮挡处、"
        "转身后的第一落脚点或主要行走轨迹，可能因发现较晚和反应距离短而具有更高绊倒风险。"
        "图像中能看见物品不等于行走者能提前看见；结合接近方向、遮挡、地面反差、低矮程度和"
        "首次可见距离判断。醒目性不能抵消通道占用或失衡后果。无法可靠判断接近方向、深度、"
        "尺寸或遮挡关系时使用unknown或insufficient，不假设老人的视力、步态、认知状态或路线。"
